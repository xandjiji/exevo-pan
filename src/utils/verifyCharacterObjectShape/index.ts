export const verifyCharacterObjectShape = (
  charObject: CharacterObject,
): boolean => {
  let status = true
  const checkProperty = (property: PropertyKey) => {
    if (!charObject.hasOwnProperty(property)) {
      console.log(`Deprecated local data [missing: '${property as string}']`)
      status = false
    }
  }
